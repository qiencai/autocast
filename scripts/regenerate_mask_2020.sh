#!/bin/bash
#SBATCH --job-name=regen_mask_2020
...
print("=" * 80)
print("REGENERATING LAND MASK FROM 2020 DATA")
print("=" * 80)

# Use 2020 data instead of 2018
nc_file = "/bask/projects/v/vjgo8416-ai-phy-sys/qqaa9560/data/seaice/raw_osisaf/osisaf_nh_2020.nc"
mask_path = Path("/bask/projects/v/vjgo8416-ai-phy-sys/qqaa9560/data/seaice/raw_osisaf/land_mask.pt")

# Backup old mask if exists
if mask_path.exists():
    backup_path = mask_path.with_suffix('.pt.old')
    print(f"\n1. Backing up old mask to: {backup_path}")
    torch.save(torch.load(mask_path), backup_path)
    print("   ✓ Backup created")

print(f"\n2. Creating mask from: {nc_file}")
print(f"   Loading netCDF file...")
ds = xr.open_dataset(nc_file)
print(f"   Dataset variables: {list(ds.data_vars)}")

# Get sea ice concentration variable
sic_var = None
...
print(f"\n3. Original mask shape: {mask_tensor.shape}")
...
print(f"\n6. Saved transposed mask to: {mask_path}")

print("\n" + "=" * 80)
print("MASK REGENERATION COMPLETE")
print("=" * 80)
print(f"Source: osisaf_nh_2020.nc")
