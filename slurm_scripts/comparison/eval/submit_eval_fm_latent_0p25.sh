...
# 25%-progress checkpoint using the default eval.mode=auto path.
#
# eval.mode=auto resolves to encode_once for processor-only cached-latent runs
# when autoencoder_checkpoint is supplied. That keeps raw-space metrics while
# avoiding the extra per-step decode->encode drift charged by the explicit
# ambient variant.
#
# Current 2026-04-20 FM runs saved legacy quarter-*.ckpt files, so this uses
# the first sorted quarter checkpoint as the 25% fallback. Future runs with
# snapshot-0p25-*.ckpt files are preferred automatically.

EVAL_BATCH_SIZE=4
EVAL_N_MEMBERS=10
TIMEOUT_MIN=360
EVAL_SUBDIR="eval_0p25"
PROGRESS_TOKEN="0p25"
PROGRESS_LABEL="0.25"
LEGACY_QUARTER_INDEX=0
...
for run_dir in "${RUN_DIRS[@]}"; do
    ...
    for run_dry in "${RUN_DRY_STATES[@]}"; do
        ...
        echo "Submitting FM cached-latent eval (${PROGRESS_LABEL} checkpoint, mode=auto -> encode_once)"
        echo "  mode: ${run_label}"
